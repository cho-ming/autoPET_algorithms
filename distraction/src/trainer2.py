...
class ModelTrainer:
    ...
    def __init__(self, model, dataloaders, criterion,fp1_loss ,fp2_loss ,fp3_loss ,fp4_loss,
                 fn1_loss ,fn2_loss ,fn3_loss ,fn4_loss,optimizer ,
        ...
        self.fp1_loss = fp1_loss
        self.fp2_loss = fp2_loss
        self.fp3_loss = fp3_loss
        self.fp4_loss = fp4_loss
        self.fn1_loss = fn1_loss
        self.fn2_loss = fn2_loss
        self.fn3_loss = fn3_loss
        self.fn4_loss = fn4_loss
        self.metric = metric
        self.recall = recall
            ...
            for phase in ['train', 'val']:
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    for sample in self.dataloaders[phase]:
                        ...
                        fp, fn = sample['fp'].to(self.device), sample['fn'].to(self.device)


                        # 1x2x144x144x144
                        ss = target[0,0,:,:,:].shape
                        fnx1 = fn_class(F.interpolate(fn, size= (96,96,96), mode='nearest'))
                        fnx2 = fn_class(F.interpolate(fn, size=(48,48,48), mode='nearest'))
                        fnx3 = fn_class(F.interpolate(fn, size=(24, 24, 24), mode='nearest'))
                        fnx4 = fn_class(F.interpolate(fn, size=(12, 12,12), mode='nearest'))
                        fpx1 = fn_class(F.interpolate(fp, size=(96,96,96), mode='nearest'))
                        fpx2 = fn_class(F.interpolate(fp, size=(48,48,48), mode='nearest'))
                        fpx3 = fn_class(F.interpolate(fp, size=(24, 24, 24), mode='nearest'))
                        fpx4 = fn_class(F.interpolate(fp, size=(12,12, 12), mode='nearest'))

                        # Forward pass:
                        output, fn1, fn2, fn3, fn4, fp1, fp2, fp3, fp4 = self.model(input)
                        ...
                        loss_fn1 = self.fn1_loss(fn1, fnx1)
                        loss_fn2 = self.fn2_loss(fn2, fnx2)
                        loss_fn3 = self.fn3_loss(fn3, fnx3)
                        loss_fn4 = self.fn4_loss(fn4, fnx4)
                        loss_fp1 = self.fp1_loss(fp1, fpx1)
                        loss_fp2 = self.fp2_loss(fp2, fpx2)
                        loss_fp3 = self.fp3_loss(fp3, fpx3)
                        loss_fp4 = self.fp4_loss(fp4, fpx4)

                        loss_all = loss + loss_fp1 + loss_fp2 + loss_fp3 + loss_fp4 + loss_fn1 + loss_fn2 + loss_fn3 + loss_fn4
                        ...
                        # Losses and metric:
                        phase_loss += loss_all.item()
                        phase_metric += metric.item()
                        phase_recall += recall.item()
                        phase_precision += precision.item()

                        with np.printoptions(precision=3, suppress=True):
                            print(f'batch: {batch} \tbatch loss: \t{loss_all:.3f} \tmetric: \t{metric:.3f} \trecall: \t{recall:.3f} \tprecision: \t{precision:.3f}')

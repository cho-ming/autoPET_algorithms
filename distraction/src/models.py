...
from .layers import BasicConv3d, FastSmoothSeNormConv3d, RESseNormConv3d, UpConv, ITN3D, DAM
...
class BaselineUNet_down3_DAM(nn.Module):
    def __init__(self, in_channels, n_cls, n_filters):
        super(BaselineUNet_down3_DAM, self).__init__()
        self.in_channels = in_channels
        self.n_cls = 1 if n_cls == 2 else n_cls
        self.n_filters = n_filters

        self.block_1_1_left = RESseNormConv3d(in_channels, n_filters, kernel_size=3, stride=1, padding=1)
        self.block_1_2_left = RESseNormConv3d(n_filters, n_filters, kernel_size=3, stride=1, padding=1)

        self.pool_1 = nn.MaxPool3d(kernel_size=2, stride=2)  # 64, 1/2
        self.block_2_1_left = RESseNormConv3d(n_filters, 2 * n_filters, kernel_size=3, stride=1, padding=1)
        self.block_2_2_left = RESseNormConv3d(2 * n_filters, 2 * n_filters, kernel_size=3, stride=1, padding=1)

        self.pool_2 = nn.MaxPool3d(kernel_size=2, stride=2)  # 128, 1/4
        self.block_3_1_left = RESseNormConv3d(2 * n_filters, 4 * n_filters, kernel_size=3, stride=1, padding=1)
        self.block_3_2_left = RESseNormConv3d(4 * n_filters, 4 * n_filters, kernel_size=3, stride=1, padding=1)

        self.pool_3 = nn.MaxPool3d(kernel_size=2, stride=2)  # 256, 1/8
        self.block_4_1_left = RESseNormConv3d(4 * n_filters, 8 * n_filters, kernel_size=3, stride=1, padding=1)
        self.block_4_2_left = RESseNormConv3d(8 * n_filters, 8 * n_filters, kernel_size=3, stride=1, padding=1)

        self.pool_4 = nn.MaxPool3d(kernel_size=2, stride=2)  # 256, 1/8
        self.block_5_1_left = RESseNormConv3d(8 * n_filters, 16 * n_filters, kernel_size=3, stride=1, padding=1)
        self.block_5_2_left = RESseNormConv3d(16 * n_filters, 16 * n_filters, kernel_size=3, stride=1, padding=1)

        self.dam4 = DAM(16*n_filters)
        self.upconv_4 = nn.ConvTranspose3d(16 * n_filters, 8 * n_filters, kernel_size=3, stride=2, padding=1,
                                           output_padding=1)
        self.block_4_1_right = RESseNormConv3d((8 + 8) * n_filters, 8 * n_filters, kernel_size=3, stride=1, padding=1)
        self.block_4_2_right = RESseNormConv3d(8 * n_filters, 8 * n_filters, kernel_size=3, stride=1, padding=1)

        self.dam3 = DAM(8 * n_filters)
        self.upconv_3 = nn.ConvTranspose3d(8 * n_filters, 4 * n_filters, kernel_size=3, stride=2, padding=1,
                                           output_padding=1)
        self.block_3_1_right = RESseNormConv3d((4 + 4) * n_filters, 4 * n_filters, kernel_size=3, stride=1, padding=1)
        self.block_3_2_right =RESseNormConv3d(4 * n_filters, 4 * n_filters, kernel_size=3, stride=1, padding=1)

        self.dam2 = DAM(4 * n_filters)
        self.upconv_2 = nn.ConvTranspose3d(4 * n_filters, 2 * n_filters, kernel_size=3, stride=2, padding=1,
                                           output_padding=1)
        self.block_2_1_right = RESseNormConv3d((2 + 2) * n_filters, 2 * n_filters, kernel_size=3, stride=1, padding=1)
        self.block_2_2_right = RESseNormConv3d(2 * n_filters, 2 * n_filters, kernel_size=3, stride=1, padding=1)

        self.dam1 = DAM(2 * n_filters)
        self.upconv_1 = nn.ConvTranspose3d(2 * n_filters, n_filters, kernel_size=3, stride=2, padding=1,
                                           output_padding=1)
        self.block_1_1_right = RESseNormConv3d((1 + 1) * n_filters, n_filters, kernel_size=3, stride=1, padding=1)
        self.block_1_2_right = RESseNormConv3d(n_filters, n_filters, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        x, fn4, fp4 = self.dam4(x)
        x = self.block_4_2_right(self.block_4_1_right(torch.cat([self.upconv_4(x), ds3], 1)))
        x, fn3, fp3 = self.dam3(x)
        x = self.block_3_2_right(self.block_3_1_right(torch.cat([self.upconv_3(x), ds2], 1)))
        x, fn2, fp2 = self.dam2(x)
        x = self.block_2_2_right(self.block_2_1_right(torch.cat([self.upconv_2(x), ds1], 1)))
        x, fn1, fp1 = self.dam1(x)
        x = self.block_1_2_right(self.block_1_1_right(torch.cat([self.upconv_1(x), ds0], 1)))

        x = self.conv1x1(x)

        if self.n_cls == 1:
            return torch.sigmoid(x), fn1, fn2, fn3, fn4, fp1,fp2,fp3, fp4
        else:
            return F.softmax(x, dim=1), fn1, fn2, fn3, fn4, fp1,fp2,fp3, fp4
